              ...
              ADC_InitTypeDef* ADC_InitStruct)
{
	...
	assert_param(IS_ADC_DATA_ALIGN(
	...
	tmpreg2 |= (uint8_t)(ADC_InitStruct->ADC_NbrOfChannel -
	                     (uint8_t)1);
                              ...
                              uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
	...
	if(ADC_Channel > ADC_Channel_9)
	{
		...
		tmpreg2 = (uint32_t)ADC_SampleTime << (3 *
		                                       (ADC_Channel - 10));
                               ...
                               uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
	...
	tmpreg2 = JSQR_JSQ_Set << (5 * (uint8_t)((Rank + 3) -
	                                         (tmpreg3 + 1)));
	/* Clear the old JSQx bits for the selected rank */
	tmpreg1 &= ~tmpreg2;
	/* Calculate the mask to set: ((Rank-1)+(4-JL-1)) */
	tmpreg2 = (uint32_t)ADC_Channel << (5 * (uint8_t)((
	                                                      Rank + 3) - (tmpreg3 + 1)));
...
void ADC_SetInjectedOffset(ADC_TypeDef* ADCx,
                           uint8_t ADC_InjectedChannel, uint16_t Offset)
{
	__IO uint32_t tmp = 0;
	/* Check the parameters */
	assert_param(IS_ADC_ALL_PERIPH(ADCx));
